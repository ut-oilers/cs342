...
def grade_all(assignment_module, logger, verbose=False):
    ...
    for g in Grader.__subclasses__():
        if g.has_cases():
            ...
            if verbose:
                logger.info(" --------------------------------------------------    [ %3d / %3d ]" % (s, ts))
            else:
                logger.info(" * %-50s  [ %3d / %3d ]" % (g.__doc__, s, ts))

            total_score += ts
            score += s

    logger.info("Total                                                    %3d / %3d" % (score, total_score))

    return score


def load_assignment(assignment_path: str, pre_import_fn=None):
    import atexit
    import importlib
    import tempfile
    import zipfile
    ...
    if assignment_path.is_dir():
        ...
        with zipfile.ZipFile(assignment_path) as f:
            ...
            if len(module_names) != 1:
                print(f"Malformed zip file, expected one top-level folder, got {len(module_names)}")
                return None
    ...
    try:
        ...
        print(f'Failed to load your solution: "{e!s}"')
...
def init_loggers(log_path: str, show_debug=False):
    formatter = RuntimeFormatter("[%(levelname)-8s %(elapsed)s] %(message)s")
...
def run():
    ...
    total_score = grade_all(assignment, logger, args.verbose)
